...
Y_OPTIONS = ["Capacity retention", "Columbic efficiency", "Energy efficiency", "Voltaic Efficiency"]
...
if enable:
    ...
    plot_limits : Dict[str, List[Union[None, float]]] = st.session_state["CellCycling_plot_limits"]
    ...
    with container_tab:
        ...
        with st.expander("Create new experiment container", expanded=True):
            ...
            st.write("""In this tab you can create new experiments containers to hold different
            ...
            with col2:
                experiments_names = st.multiselect("Select the experiments to add to the container", status.get_experiment_names(), key="add_experiment_to_new")

            with col3:
                container_color = st.color_picker("Select the container color", value=get_plotly_color(len(available_containers)))
            

            apply = st.button("➕ Create container", key="create container", disabled=True if container_name=="" else False)                    
        ...
        if available_containers != []:
            ...
            with st.expander("Edit experiment container", expanded=False):
                ...
                selected_container_name = st.selectbox("Select the container to edit", [obj.name for obj in available_containers])
                ...
                if selected_container_name != None:
                    ...
                    container_idx : int = [container.name for container in available_containers].index(selected_container_name)
                    ...
                    with col1:
                        valid_exp_names = [name for name in status.get_experiment_names() if name not in selected_container.get_experiment_names]
                        experiment_name = st.selectbox("Select the experiments to add to the container", valid_exp_names)

                    with col2:
                        st.write("")
                        st.write("")
                        add = st.button("➕ Add experiment", disabled=True if experiment_name is None else False)
                    ...
                    with col1:
                        get_experiment_names = st.multiselect("Select the experiments to remove from the container", [name for name in selected_container.get_experiment_names], key="add_experiment_to_existing")

                    with col2:
                        st.write("")
                        st.write("")
                        remove = st.button("➖ Remove experiment", disabled=True if get_experiment_names == [] else False)
    ...
    with plot_tab:
        ...
        st.write("""In this tab you can create a cell cycling plot and interactively selecting
            its appearence""")
        ...
        if plot_limits["x"][0] != None:
            ...
            with st.expander("Annotation editor", expanded=False):
                ...
                with col1:
                    annotation_size = st.number_input("Enter the dimension of the annotation font", min_value=4, value=12)

                with col2:
                    annotation_color = st.color_picker("Select annotation color", value="#000000")
                ...
                with col2:
                    if mode == "Add new":
                        ...
                        annotation = st.selectbox("Select annotation", [text for text in annotation_dict.keys()])
                
                with col3:
                    x_position = st.slider("X position", min_value=float(plot_limits["x"][0]), max_value=float(plot_limits["x"][1]), step=0.1)
                
                with col4:
                    y_position = st.slider("Y position", min_value=float(plot_limits["y"][0]), max_value=float(plot_limits["y"][1]), step=0.1)

                col1, col2 = st.columns(2)

                with col1:
                    if mode == "Edit existing":
                        remove = st.button("❌ Remove annotation", key="annotation_remove", disabled=True if annotation is None else False)
                    pass
                
                with col2:
                    if mode == "Add new":
                        apply = st.button("✅ Apply", key="annotation_apply", disabled=True if annotation == "" else False)
        ...
        with col2:
            ...
            primary_axis_name = st.selectbox("Select the dataset for the primary Y axis", Y_OPTIONS)
            secondary_axis_name = st.selectbox("Select the dataset for the secondary Y axis", [option for option in Y_OPTIONS if option != primary_axis_name])

            st.markdown("###### Graph options")
            primary_axis_marker = st.selectbox("Select primary Y axis markers", [m for m in MARKERS.keys()])
            secondary_axis_marker = st.selectbox("Select secondary Y axis markers", [m for m in MARKERS.keys() if m != primary_axis_marker])
            which_grid = st.radio("Y-axis grid selector", options=["Primary", "Secondary", "None"])
        ...
        with col1:
            ...
            for container in available_containers:
                ...
                for name, cellcycling in container:
                    ...
                    fig.add_trace(go.Scatter(x=cycle_index, y=primary_axis, name=container.name, mode="markers", marker_symbol=primary_marker, line=dict(color=container.hex_color)), secondary_y=False)
                    fig.add_trace(go.Scatter(x=cycle_index, y=secondary_axis, name=container.name, mode="markers", marker_symbol=secondary_marker, line=dict(color=container.hex_color), showlegend=False), secondary_y=True)
            
            if annotation_dict != {}:
                print(annotation_dict)
                for text, position in annotation_dict.items():
                    fig.add_annotation(x=position[0], y=position[1], text=text, font_size=annotation_size ,font_color=annotation_color, showarrow=False)
